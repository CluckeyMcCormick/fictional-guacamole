...
func _on_SunflowerArea_body_entered(body):
    ...
    $SunflowerArea.monitoring = false
    $SunflowerArea.monitorable = false
    $SunflowerArea/SunflowerShape.disabled = true
    
    # Hover the corn .002 off of the ground
    $SunflowerQuad.translate( Vector3(0, -0.998, 0) )
    # Reset the corns's y rotation so that we can do the other rotations
    $SunflowerQuad.rotate_y(-PI / 4)
    # Rotate the corn so that it's flat
    $SunflowerQuad.rotate_x(-PI / 2)
